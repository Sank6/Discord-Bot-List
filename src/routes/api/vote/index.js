...
route.patch("/:id", auth, async (req, res) => {
  ...
  await Bots.updateOne({ botid: req.params.id }, { $inc: { vote: 1 } })
  ...
  let channel = await req.app.get('client').channels.cache.get(server.vote_log);
  let webhook = (await channel.fetchWebhooks()).first();
  if (!webhook) 
    webhook = await channel.createWebhook('Discord Bot List')
  await webhook.send(`<@${req.user.id}> (${userProfile.tag}) has voted for <@${req.params.id}>`);
