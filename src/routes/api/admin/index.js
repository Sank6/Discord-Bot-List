...
route.patch("/:id", auth, async function (req, res) {
    ...
    const member = await req.app.get('client').guilds.cache.get(server.id).members.fetch(req.user.id);
    if (!server.admin_user_ids.includes(req.user.id) && !member.roles.cache.has(bot_verifier)) return res.json({ success: false, message: 'Invalid User' });
