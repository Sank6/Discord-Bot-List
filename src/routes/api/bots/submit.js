...
route.post("/", auth, async (req, res) => {
    ...
    try {
        await req.app.get('client').channels.cache.find(c => c.id === server.mod_log_id).send(`<@${owners[0]}> submitted <@${bot.id}>: <@&${server.role_ids.bot_verifier}>`);
